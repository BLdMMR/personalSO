...
void queue_init(PQUEUE queue) {
    ...
    initializeListHead(&(queue->head), NULL);
    queue->size = 0;
}
...
void queue_offer(PQUEUE queue_ int val) {
    PQUEUE_NODE node = (PQUEUE_NODE)malloc(sizeof (PQUEUE_NODE));
    node->value = val;
...
int queue_take(PQUEUE queue) {
    ...
    pthread_mutex_unlock(&(queue->lock));
    if (pentry != NULL) {
        PQUEUE_NODE node = container_of(pentry, PQUEUE_NODE, entry);
        int val = node->value;
        free(node);
        return val;
    }
